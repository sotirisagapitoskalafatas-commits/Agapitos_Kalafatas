...
delete from public.communications where lead_id like 'a0000000-0000-4000-8000-0000000000__';
delete from public.calendar_events where lead_id like 'a0000000-0000-4000-8000-0000000000__';
...
delete from public.deals      where id like 'b0000000-0000-4000-8000-0000000000__';
delete from public.leads      where id like 'a0000000-0000-4000-8000-0000000000__';

-- ------------------------------------------------------------------
-- 1) LEADS  (status: new_lead / contacted / customer / archived)
-- Service categories match the site contact form options.
-- ------------------------------------------------------------------
insert into public.leads (id, first_name, last_name, email, phone, property_type, region, service_category, comments, gdpr_consent, status, created_at) values
 ('a0000000-0000-4000-8000-000000000001','Maria','Papadopoulou','maria.papadopoulou@gmail.com','6912345678','House','Attica','Electricity','Looking to switch provider, wants the cheapest tariff', true, 'new_lead', now() - interval '2 days'),
 ('a0000000-0000-4000-8000-000000000002','Giorgos','Nikolaou','g.nikolaou@outlook.com','6977001122','House','Thessaloniki','Natural Gas','Needs a new gas connection and supplier quote', true, 'new_lead', now() - interval '1 day'),
 ('a0000000-0000-4000-8000-000000000003','Elena','Georgiou','elena.georgiou@nova-industrial.gr','6944556677','Business','Attica','Solar','Wants a 5kW rooftop PV system for her bakery', true, 'contacted', now() - interval '9 days'),
 ('a0000000-0000-4000-8000-000000000004','Dimitris','Antoniou','d.antoniou@buildco.gr','6988001122','Business','Crete','Energy Savings','Large facility — energy audit + savings plan', true, 'contacted', now() - interval '12 days'),
 ('a0000000-0000-4000-8000-000000000005','Katerina','Vasilaki','k.vasilaki@gmail.com','6900112233','House','Attica','Energy Storage','Adding a battery to existing solar install', true, 'new_lead', now() - interval '3 days'),
 ('a0000000-0000-4000-8000-000000000006','Nikos','Kostopoulos','n.kostopoulos@yacht-marine.gr','6933221100','Business','Aegean Islands','E-Mobility','Wants marina EV chargers + fleet charging', true, 'contacted', now() - interval '15 days'),
 ('a0000000-0000-4000-8000-000000000007','Sofia','Dimitriou','sofia.d@corp-solutions.gr','6966554433','Business','Attica','Website','Full company website redesign, urgent before expo', true, 'contacted', now() - interval '5 days'),
 ('a0000000-0000-4000-8000-000000000008','Panagiotis','Roussos','p.roussos@med.gr','6955443322','Business','Thessaly','Custom Software','Clinic scheduling system for 3 branches', true, 'new_lead', now() - interval '6 days'),
 ('a0000000-0000-4000-8000-000000000009','Anna','Makri','anna.makri@hotmail.com','6999888777','House','Peloponnese','Electricity','Compare tariffs for a 2-person household', true, 'customer', now() - interval '60 days'),
 ('a0000000-0000-4000-8000-000000000010','Christos','Stavrou','ch.stavrou@land.gr','6922113344','House','Macedonia','Solar','Self-consumption PV + storage, family home', true, 'customer', now() - interval '45 days'),
 ('a0000000-0000-4000-8000-000000000011','Ioanna','Liakopoulou','ioanna.l@retail-group.gr','6977558899','Business','Attica','E-shop','Wants to move legacy webshop to a modern storefront', true, 'customer', now() - interval '30 days'),
 ('a0000000-0000-4000-8000-000000000012','Vasilis','Pappas','v.pappas@asfalistiki.gr','6955001122','Business','Ionian Islands','Car Insurance','Fleet auto insurance renewal for 12 vans', true, 'customer', now() - interval '20 days');
